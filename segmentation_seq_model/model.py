    ...
    def forward(self, x):
        if not self.training:
            ...
            if self.buffer.shape[0] != b or self.buffer.shape[2] != h:
                self.buffer = torch.zeros(b, self.fold, h, w, device=x.device, dtype=x.dtype)
...
class PredictionHead(nn.Module):
    ...
    def forward(self, features):
        ...
        for i, x in enumerate(features):  #
            ...
            if i == 0:
                pred_normals = self.normal_conv(x_with_coords)
        ...
        return pred_logits, pred_boxes, pred_coeffs, pred_normals
...
class FastMelonSegmenter(nn.Module):
    ...
    def forward(self, x):
        ...
        pred_logits, pred_boxes, pred_coeffs, pred_normals = self.head([p3, p4, p5])

        # 최종 모델 출력 완성!
        # 학습 시: 이 출력값들과 정답(Ground Truth)을 헝가리안 매칭하여 Loss 계산
        # 추론 시: threshold 넘는 것만 골라내어 einsum으로 마스크 조립
        return pred_logits, pred_boxes, pred_coeffs, prototypes, pred_normals
...
if __name__ == "__main__":
    ...
    summary(model, input_size=(1, 4, 256, 448))
